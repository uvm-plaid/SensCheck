...
smap f (SList_UNSAFE l) = SList_UNSAFE $ (wrap @b) . unwrap . f <$> l

-- Previous unworking smap
smap_ :: forall fn_sens a b s2 m.
   (forall s1. a s1 -> b (ScaleSens s1 fn_sens))
   -> SList m a s2
   -> SList m b (ScaleSens s2 (MaxNat fn_sens 1))
...
sfilter :: (forall s. Double -> Bool)
  -> L1List (SDouble m) s1
  -> L1List (SDouble m) s1
sfilter f xs = cong scale_unit $ sfoldr_ @1 @1 (sfilter_fn f) (sConstL @'[] []) xs

count :: SList cm t s -> SDouble Diff s
count xs = cong scale_unit $ sfoldr_ @1 @1 count_fn (sConstD @'[] 0) xs
