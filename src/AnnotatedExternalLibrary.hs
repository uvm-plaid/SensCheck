...
unsafe_f_prop :: Double -> Double -> Double -> Double -> Bool
unsafe_f_prop x1 y1 x2 y2 =
  let d1 = abs (x1 - x2)
      d2 = abs (y1 - y2)
...
f_prop :: SDouble Diff '[] -> SDouble Diff '[] -> SDouble Diff '[] -> SDouble Diff '[] -> Bool
f_prop a1 a2 b1 b2 =
...
add_solo :: SDoubleMatrixL2 s1 -> SDoubleMatrixL2 s2 -> SDoubleMatrixL2 (s1 +++ s2)
add_solo m1 m2 =
  SMatrix_UNSAFE $
    D_UNSAFE
...
prop_safe_add_solo a1 a2 b1 b2 =
  let d1 = norm_2 $ toDoubleMatrix a1 - toDoubleMatrix a2 -- L2 distance between first arguments
      ...
      -- L2 distance between two outputs
      dout = norm_2 $ toDoubleMatrix (add_solo a1 b1) - toDoubleMatrix (add_solo a2 b2)
   in dout <= d1 + d2 + 0.000000001
